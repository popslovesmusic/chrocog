...
class BenchmarkRunner:
    ...
    def __init__(self, config: Optional[BenchmarkConfig]: Dict = None) -> None:
    ...
    def run_full_benchmark(self) -> BenchmarkResults:
        ...
        logger.info("Test 1: WebSocket Latency Benchmark")
        logger.info("-" * 70)
        latency_stats, latency_dist = self._run_latency_benchmark()
        logger.info(str())

        # Test 2: Frame Rate Benchmark
        logger.info("Test 2: Sustained Frame Rate Benchmark")
        logger.info("-" * 70)
        fps_stats = self._run_fps_benchmark()
        logger.info(str())

        # Test 3: Resource Usage Benchmark
        logger.info("Test 3: Resource Usage & Stability Benchmark")
        logger.info("-" * 70)
        ...
        if self.scaler:
            logger.info("Test 4: Adaptive Scaling Benchmark")
            logger.info("-" * 70)
    ...
    def _run_latency_benchmark(self) -> tuple:
        ...
        """
        logger.info("  Running %s latency measurements...", self.config.latency_test_frames)
        ...
        for i in range(self.config.latency_test_frames):
            ...
            if (i + 1) % 1000 == 0:
                logger.info("    Progress: %s/%s", i + 1, self.config.latency_test_frames)
        ...
        logger.info("  Avg latency: %s ms", stats['avg_latency_ms']:.2f)
        logger.info("  Max latency: %s ms", stats['max_latency_ms']:.2f)
        logger.info("  P95 latency: %s ms", stats['p95_latency_ms']:.2f)
    ...
    def _run_fps_benchmark(self) -> Dict:
        ...
        logger.info("  Running sustained FPS test (%ss)...", test_duration:.0f)
        ...
        logger.info("  Average FPS: %s", avg_fps:.1f)
        logger.info("  Frame drops: %s (%s%)", frame_drops, frame_drop_rate:.1f)
    ...
    def _run_resource_benchmark(self) -> tuple:
        ...
        """
        test_duration = 60.0  # 60 seconds for faster testing
        logger.info("  Monitoring resources (%ss)...", test_duration:.0f)
        ...
        logger.info("  CPU: avg=%s%, max=%s%", cpu_stats['avg_cpu_percent']:.1f, cpu_stats['max_cpu_percent']:.1f)
        logger.info("  Memory: increase=%sMB (%s%)", memory_increase:.1f, memory_increase_percent:.1f)
    ...
    def _run_adaptive_scaling_benchmark(self) -> Dict:
        ...
        Returns:
            Scaling statistics
        """
        logger.info("  Testing adaptive scaling...")

        # Simulate performance degradation
        logger.info("    Phase 1: Low performance (triggering scale down)")
        for i in range(20):
        ...
        logger.info("    Phase 2: High performance (triggering scale up)")
        for i in range(30):
            metrics = PerformanceMetrics(
                timestamp=time.time(),
        ...
        logger.info("  Scale adjustments: %s", stats['total_adjustments'])
        logger.info("  Final complexity level: %s", stats['current_complexity_level'])
    ...
    def _print_summary(self) -> Any:
        ...
        if all_passed:
            ...
            logger.error("[FAIL] SOME CRITERIA NOT MET - Review results above")
    ...
    def save_results(self) -> Any:
        ...
        logger.info("Performance profile saved to: %s", profile_path)
