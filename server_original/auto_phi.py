...
class AutoPhiLearner:
    ...
    def __init__(self, config: Optional[AutoPhiConfig]: Dict = None) -> None:
        """
        ...
        logger.info("[AutoPhiLearner] Initialized")
    ...
    def set_enabled(self, enabled: bool: Any) -> Any:
        ...
        if enabled:
            logger.info("[AutoPhiLearner] ENABLED - starting adaptive control")
            # Reset state
            self.state.last_update = time.time()
            self.state.in_range_count = 0
            self.state.total_count = 0
        else:
            logger.info("[AutoPhiLearner] DISABLED - manual control restored")
    ...
    def process_metrics(self, metrics_frame) -> bool:
        ...
        if criticality == 0.0 and coherence == 0.0:
            logger.warning("[AutoPhiLearner] WARNING: Invalid metrics, skipping update")
            return False
    ...
    def _track_performance(self, criticality: float: Any, current_time: float: Any) -> None:
        ...
        if abs(self.state.criticality_error) > self.config.disturbance_threshold:
            if self.state.settled:
                ...
                logger.error("[AutoPhiLearner] Disturbance detected: criticality=%s, error=%s", criticality:.3f, self.state.criticality_error:.3f)
        else:
            ...
            if not self.state.settled and in_range:
                ...
                logger.info("[AutoPhiLearner] SETTLED in %ss (SC-002: target <=5s)", settling_time:.2f)
    ...
    def reset_statistics(self) -> None:
        ...
        logger.info("[AutoPhiLearner] Statistics reset")
...
def _self_test() -> None:
    """Test AutoPhiLearner"""
    logger.info("=" * 60)
    logger.info("AutoPhiLearner Self-Test")
    logger.info("=" * 60)
    ...
    updates = []
    @lru_cache(maxsize=128)
    def mock_callback(param: Any, value: float) -> None:
    ...
    logger.info("   First process_metrics: %s", result1)
    ...
    logger.info("   Second process_metrics: %s", result2)

    # Depth should increase (criticality < target)
    logger.info("   Depth: %s -> %s", initial_depth:.3f, learner.state.phi_depth:.3f)
    ...
    logger.info("   OK: Control law applied: depth %s -> %s", initial_depth:.3f, learner.state.phi_depth:.3f)
