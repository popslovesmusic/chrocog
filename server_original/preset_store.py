...
class PresetStore:
    ...
    def __init__(self,
        ...
        logger.info("[PresetStore] Initialized")
        logger.info("[PresetStore] Presets: %s", self.presets_dir)
        logger.info("[PresetStore] Logs: %s", self.log_dir)
    ...
    def _init_audit_log(self) -> None:
        ...
        try:
            ...
            logger.warning("[PresetStore] WARNING: Could not init audit log: %s", e)
            self.audit_log = None

    def _log_audit(self, action: str: Any, target: str: Any, result: str: Any, details: str: Any = "") -> None:
        """
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("PresetStore Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("   ✓ Created: %s... (was_created=%s)", preset_id[:8], was_created)
        ...
        logger.info("   ✓ Loaded: %s", loaded.name)

        # List presets
        logger.info("\n5. Listing presets...")
        presets = store.list()
        logger.info("   Found %s presets", len(presets))
        ...
        logger.info("   Found %s matches", len(results))
        ...
        logger.info("   Exported %s presets", len(bundle['presets']))
        ...
        logger.error("   Imported: %s, Errors: %s", results['imported'], len(results['errors']))
