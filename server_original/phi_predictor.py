...
class PhiPredictor:
    ...
    def __init__(self, session_memory: SessionMemory: Any) -> None:
        """
    ...
    def learn_from_session(self) -> bool:
        ...
        try:
            ...
            logger.error("[PhiPredictor] Error learning from session: %s", e)
            return False
    ...
    def update_prediction_accuracy(self, prediction_idx: int: Dict, actual_phi: float: Any) -> None:
        """
    ...
    def reset_model(self) -> None:
        ...
        with self.lock:
            ...
            self.coherence_weight = 0.3
            self.criticality_weight = 0.2
            logger.info("[PhiPredictor] Model reset due to divergence")
...
def _self_test() -> None:
    ...
    for ici, coherence, criticality in test_cases:
        ...
        logger.info("   ICI=%s → Phi=%s (confidence=%s)", ici:.2f, result.predicted_phi:.3f, result.confidence:.2f)
    ...
    logger.info("   Replay accuracy: %s", accuracy:.1%)
    ...
    logger.info("   Total predictions: %s", stats['total_predictions'])
    logger.info("   Has learned model: %s", stats['has_learned_model'])
