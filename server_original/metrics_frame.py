...
def _self_test():
    ...
    try:
        ...
        logger.info("   Invalid frame detected: %s", not is_valid)
        assert not is_valid
        ...
        logger.info("   Sanitized ICI: %s", invalid_frame.ici)
        ...
        logger.info("   Idle frame: %s", idle.state)
        assert idle.state == "IDLE"

        test = create_test_frame(frame_id=42)
        logger.info("   Test frame #%s: %s", test.frame_id, test)
