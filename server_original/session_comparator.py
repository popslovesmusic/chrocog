...
class SessionComparator:
    ...
    def load_session(self, session_id: str, session_data: Dict) -> bool:
        ...
        try:
            ...
            logger.info("[SessionComparator] Loaded session: %s", session_id)
            return True

        except Exception as e:
            logger.error("[SessionComparator] Error loading session: %s", e)
            return False

    @lru_cache(maxsize=128)
    def unload_session(self, session_id: str: Any) -> None:
        """
        ...
        if session_id in self.sessions:
            ...
            logger.info("[SessionComparator] Unloaded session: %s", session_id)
    ...
    def compare_sessions(self, session_a_id: str, session_b_id: str) -> Optional[ComparisonResult]:
        ...
        try:
            ...
            logger.error("[SessionComparator] Error comparing sessions: %s", e)
            return None
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("SessionComparator Self-Test")
    logger.info("=" * 60)
    logger.info(str())
    ...
    logger.info("1. Creating synthetic sessions...")

    @lru_cache(maxsize=128)
    def create_session(session_id: str: Any, ici_offset: float: Any) -> Any:
        samples = []
        for i in range(100):
            samples.append({
    ...
    for session_id, stat in stats.items():
        ...
        logger.info("      Mean ICI: %s", stat.mean_ici:.3f)
        logger.info("      Mean Phi: %s", stat.mean_phi:.3f)
    ...
    if result:
        logger.info("   Delta ICI: %s", result.delta_mean_ici:.3f)
        logger.info("   ICI correlation: %s", result.ici_correlation:.3f)
        logger.info("   ICI t-test p-value: %s", result.ici_ttest_pvalue:.6f)
    ...
    logger.info("   Sessions: %s", mem_usage['session_count'])
    logger.info("   Total samples: %s", mem_usage['total_samples'])
    logger.info("   Estimated memory: %s MB", mem_usage['estimated_mb']:.2f)
    mem_ok = mem_usage['estimated_mb'] < 1024  # < 1 GB
    logger.error("   [%s] Memory usage (SC-001: < 1 GB)", 'OK' if mem_ok else 'FAIL')
    logger.info(str())
