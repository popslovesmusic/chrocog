...
class StateMemory:
    ...
    def __init__(self, config: Optional[StateMemoryConfig]: Dict = None) -> None:
        """
    ...
    def set_enabled(self, enabled: bool: Any) -> Any:
        ...
        if enabled:
            logger.info("[StateMemory] ENABLED - temporal memory active")
        else:
            logger.info("[StateMemory] DISABLED - baseline behavior restored")
    ...
    def reset_buffer(self) -> None:
        ...
        logger.info("[StateMemory] Buffer reset")
...
def _self_test() -> None:
    ...
    logger.info("   OK: Added 50 frames, buffer size = %s", len(memory.buffer))
    ...
    logger.error("   d(criticality)/dt = %s", trend_summary['d_criticality_dt']:.4f)
    logger.error("   Predicted criticality = %s", trend_summary['predicted_criticality']:.3f)
    logger.info("   Confidence = %s", trend_summary['confidence']:.3f)
    logger.info("   Current bias = %s", trend_summary['current_bias']:.3f)

    # Should detect upward trend
    assert trend_summary['d_criticality_dt'] > 0, "Should detect upward trend"
    logger.info("   OK: Trend detection working")
    ...
    assert result == False
    assert len(memory.buffer) == 0
    logger.info("   OK: Enable/disable working")
