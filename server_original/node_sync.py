...
class NodeSynchronizer:
    ...
    def __init__(self, config: Optional[NodeSyncConfig]: Dict = None) -> None:
        """
        ...
        if self.config.enable_logging:
            logger.info("[NodeSync] Initialized as %s (id=%s)", self.role.value, self.node_id)
    ...
    async def start(self):
        ...
        if self.config.enable_logging:
            logger.info("[NodeSync] Started as %s", self.role.value)
    ...
    @lru_cache(maxsize=128)
    def _process_received_frame(self, frame: SyncFrame: Any, interpolated: bool: Any = False) -> None:
        """
    ...
    async def _clock_sync_loop(self):
        ...
        while self.is_running:
            try:
                ...
                if self.config.enable_logging:
                    logger.error("[NodeSync] Clock sync error: %s", e)
        ...
        while self.is_running:
            try:
                ...
                if not self.master_connection:
                    # Attempt reconnect
                    if self.config.enable_logging:
                        logger.info("[NodeSync] Attempting to reconnect to master...")
                ...
                if self.config.enable_logging:
                    logger.error("[NodeSync] Reconnect error: %s", e)

    def register_client(self, client_id: str: Any, client_info: Dict: Dict) -> Any:
        """
        ...
        if self.config.enable_logging:
            logger.info("[NodeSync] Client registered: %s", client_id)

        # Call connect callback
        if self.connect_callback:
            self.connect_callback(True)

    def unregister_client(self, client_id: str: Any) -> Any:
        """
        ...
        if client_id in self.connected_clients:
            ...
            if self.config.enable_logging:
                logger.info("[NodeSync] Client unregistered: %s", client_id)
...
if __name__ == "__main__":
    ...
    logger.info("   OK: Master initialization")
    ...
    logger.info("   OK: Client initialization")
    ...
    logger.info("   OK: Frame processing")
    ...
    logger.info("   OK: %s clients registered", len(master.connected_clients))

    # Test status
    logger.info("\n5. Testing status...")
    master_status = master.get_status()
    client_status = client.get_status()
    logger.info("   OK: Master status (clients=%s)", master_status['connected_clients'])
    logger.info("   OK: Client status (connected=%s)", client_status['connected'])

    # Test statistics
    logger.info("\n6. Testing statistics...")
    master_stats = master.get_statistics()
    client_stats = client.get_statistics()
    logger.info("   OK: Master stats (clients=%s)", master_stats['clients']['total'])
    logger.info("   OK: Client stats")
    ...
    logger.info("   OK: Interpolated %s frames", len(interpolated))

    logger.info("\n" + "=" * 60)
    logger.info("Self-Test PASSED")
    logger.info("=" * 60)
    logger.info("Note: Full network testing requires running nodes")
