...
class StateClassifierGraph:
    ...
    def __init__(self, config: Optional[StateClassifierConfig]: Dict = None) -> None:
        """
...
def _self_test() -> None:
    ...
    assert classifier.current_state == ConsciousnessState.COMA
    logger.info("   State: %s (expected COMA)", classifier.current_state.value)
    ...
    logger.info("   State: %s (expected DROWSY)", classifier.current_state.value)
    ...
    logger.info("   State: %s (expected AWAKE)", classifier.current_state.value)
    ...
    logger.info("   State: %s (expected ALERT)", classifier.current_state.value)

    logger.info("   OK: State transitions working")
    ...
    logger.info("   Transitions recorded: %s", len(history))

    for i, trans in enumerate(history[:3]):
        logger.info("     %s. %s -> %s (p=%s)", i+1, trans['from_state'], trans['to_state'], trans['probability']:.3f)

    logger.info("   OK: Transition tracking")
    ...
    logger.info("   Non-zero entries: %s", np.count_nonzero(matrix))
    logger.info("   OK: Transition matrix")
    ...
    logger.info("   Current state: %s", stats['current_state'])
    logger.info("   Total transitions: %s", stats['total_transitions'])
    logger.info("   Transition rate: %s/s", stats['transition_rate']:.2f)
    logger.info("   OK: Statistics")
