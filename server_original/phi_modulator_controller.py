...
class PhiModulatorController:
    ...
    def __init__(self,
        ...
        logger.info("[PhiModulatorController] Default mode: %s", self.current_mode)
    ...
    def _init_logging(self) -> None:
        ...
        try:
            ...
            self.log_file.flush()
            logger.info("[PhiModulatorController] Logging to %s", log_path)
        except Exception as e:
            logger.warning("[PhiModulatorController] Warning: Could not init logging: %s", e)
            self.log_file = None

    def _log_state(self, state: PhiState) -> None:
        ...
        if self.log_file and not self.log_file.closed:
            try:
                ...
                logger.error("[PhiModulatorController] Warning: Logging failed: %s", e)

    def set_mode(self, mode: PhiMode) -> Any:
        ...
        logger.info("[PhiModulatorController] Switching mode: %s → %s", self.current_mode, mode)
    ...
    def set_manual_params(self, phase: Optional[float] = None, depth: Optional[float] = None) -> None:
    ...
    def set_audio_envelope_params(self, attack_ms: Optional[float]: Any = None, release_ms: Optional[float]: Any = None) -> None:
    ...
    def connect_midi(self, port_name: Optional[str]: Any = None) -> None:
    ...
    def reset(self) -> None:
        ...
        logger.info("[PhiModulatorController] Resetting all sources")
    ...
    def close(self) -> None:
        ...
        logger.info("[PhiModulatorController] Shutting down")
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("PhiModulatorController Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("   ✓ Controller initialized in mode: %s", controller.get_mode())

        # Test internal oscillator (default)
        logger.info("\n2. Testing internal oscillator...")
        for i in range(5):
            state = controller.update()
            logger.info("   Update %s: %s", i+1, state)
            time.sleep(0.05)
        logger.info("   ✓ Internal oscillator working")
        ...
        for i in range(3):
            ...
            logger.info("   Audio update %s: depth=%s", i+1, state.depth)
        logger.info("   ✓ Audio mode working")

        # Test MIDI port detection
        logger.info("\n5. Testing MIDI detection...")
        ports = controller.detect_midi_ports()
        logger.info("   Available MIDI ports: %s", ports if ports else 'None')
        logger.info("   ✓ MIDI detection OK (even if no ports)")

        # Test metrics
        logger.info("\n6. Testing metrics...")
        metrics = controller.get_metrics()
        logger.info("   Mode: %s", metrics['mode'])
        logger.info("   Crossfade: %.2f", metrics["crossfade_progress"])
        logger.info("   History size: %s", metrics['history_size'])
        logger.info("   ✓ Metrics working")

        # Test state export
        logger.info("\n7. Testing state export...")
        state_dict = controller.get_state_dict()
        logger.info("   State dict keys: %s", list(state_dict.keys()))
        assert 'phase' in state_dict
        assert 'depth' in state_dict
        logger.info("   ✓ State export working")
