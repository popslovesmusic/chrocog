...
class HybridBridge:
    ...
    def __init__(self, port: Optional[str]: Any = None, baudrate: int: Any = 115200, enable_logging: bool: Any = True) -> None:
        ...
        if self.enable_logging:
            logger.info("[HybridBridge] Initialized")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.port is None:
                ...
                for device in devices:
                    ...
                    if 'teensy' in desc or 'arduino' in desc or 'raspberry' in desc:
                        ...
                        if self.enable_logging:
                            logger.info("[HybridBridge] Auto-detected: %s on %s", device['description'], self.port)
                        break

                if self.port is None:
                    if self.enable_logging:
                        logger.info("[HybridBridge] No compatible device found")
                    return False
            ...
            if self.enable_logging:
                logger.info("[HybridBridge] Connected to %s @ %s baud", self.port, self.baudrate)

            return True

        except Exception as e:
            if self.enable_logging:
                logger.error("[HybridBridge] Connection error: %s", e)
            return False
    ...
    def disconnect(self) -> Any:
        ...
        if self.enable_logging:
            logger.info("[HybridBridge] Disconnected")
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            if response == self.RESP_OK:
                ...
                if self.enable_logging:
                    logger.info("[HybridBridge] Node started")
                return True
            else:
                if self.enable_logging:
                    logger.error("[HybridBridge] Start command failed")
                return False

        except Exception as e:
            if self.enable_logging:
                logger.error("[HybridBridge] Start error: %s", e)
            return False
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            if self.enable_logging:
                logger.info("[HybridBridge] Node stopped")

            return (response == self.RESP_OK)

        except Exception as e:
            if self.enable_logging:
                logger.error("[HybridBridge] Stop error: %s", e)
            return False
    ...
    def get_dsp_metrics(self) -> Dict:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Get DSP metrics error: %s", e)
            return {}
    ...
    def get_safety(self) -> Dict:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Get safety error: %s", e)
            return {}
    ...
    def set_preamp_gain(self, gain: float) -> bool:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Set preamp gain error: %s", e)
            return False
    ...
    def set_control_voltage(self, cv: ControlVoltage) -> bool:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Set control voltage error: %s", e)
            return False
    ...
    def calibrate(self) -> Optional[Dict]:
        ...
        try:
            if self.enable_logging:
                logger.info("[HybridBridge] Starting calibration (this may take 10-20 seconds)...")
            ...
            if response == self.RESP_OK:
                ...
                if len(data) >= 16:
                    ...
                    if self.enable_logging:
                        logger.info("[HybridBridge] Calibration complete")
                        logger.info("  Total latency: %s µs", total_latency_us)
                        logger.error("  SC-001 (≤2000 µs): %s", 'PASS' if total_latency_us <= 2000 else 'FAIL')
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Calibration error: %s", e)
            return None
    ...
    def get_statistics(self) -> Dict:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Get statistics error: %s", e)
            return {}
    ...
    def reset_statistics(self) -> bool:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Reset statistics error: %s", e)
            return False
    ...
    def set_mode(self, mode: HybridNodeMode) -> bool:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Set mode error: %s", e)
            return False
    ...
    def emergency_shutdown(self, reason: str) -> bool:
        ...
        try:
            if self.enable_logging:
                logger.info("[HybridBridge] EMERGENCY SHUTDOWN: %s", reason)
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Emergency shutdown error: %s", e)
            return False
    ...
    def get_version(self) -> str:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[HybridBridge] Get version error: %s", e)
            return "Unknown"
    ...
    def _metrics_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                if self.enable_logging:
                    logger.error("[HybridBridge] Metrics loop error: %s", e)
                time.sleep(0.1)
