...
class StateSyncManager:
    ...
    def __init__(self, config: Optional[SyncConfig]: Dict = None) -> None:
    ...
    def pause(self) -> None:
        ...
        if not self.paused:
            ...
            if self.config.enable_logging:
                logger.info("[StateSyncManager] Paused at master time %ss", self.get_master_time():.3f)
    ...
    def resume(self) -> None:
        ...
        if self.paused and self.pause_time:
            ...
            if self.config.enable_logging:
                logger.info("[StateSyncManager] Resumed after %ss pause", pause_duration:.3f)

    def update_state(self, **kwargs: List) -> None:
        """
    ...
    async def register_client(self, websocket):
        ...
        async with self.ws_lock:
            ...
            if self.config.enable_logging:
                logger.info("[StateSyncManager] Client registered (%s total)", len(self.ws_clients))
    ...
    async def unregister_client(self, websocket):
        ...
        async with self.ws_lock:
            if websocket in self.ws_clients:
                ...
                if self.config.enable_logging:
                    logger.info("[StateSyncManager] Client unregistered (%s total)", len(self.ws_clients))
        ...
        async with self.ws_lock:
            ...
            for client in self.ws_clients:
                try:
                    ...
                    if self.config.enable_logging:
                        logger.info("[StateSyncManager] WebSocket send timeout")
                    disconnected.append(client)
                except Exception as e:
                    if self.config.enable_logging:
                        logger.error("[StateSyncManager] WebSocket send error: %s", e)
                    disconnected.append(client)
    ...
    def start_monitoring(self) -> Any:
        ...
        if self.config.enable_logging:
            logger.info("[StateSyncManager] Monitoring started")
    ...
    def stop_monitoring(self) -> None:
        ...
        if self.config.enable_logging:
            logger.info("[StateSyncManager] Monitoring stopped")
        ...
        while self.is_running:
            try:
                ...
                if not health['meets_sc004'] and self.config.enable_logging:
                    logger.warning("[StateSyncManager] WARNING: Desync detected: %sms", health['max_state_diff_ms']:.1f)
                ...
                if self.config.enable_logging:
                    logger.error("[StateSyncManager] Monitor error: %s", e)
...
def _self_test() -> None:
    ...
    # Test 1: Master clock
    logger.info("1. Testing Master Clock (FR-002)...")
    ...
    logger.info("   Time delta: %ss (expected ~0.1s)", t2 - t1:.3f)
    logger.error("   [%s] Master clock (FR-002)", 'OK' if time_ok else 'FAIL')
    logger.info(str())

    # Test 2: Pause/Resume
    logger.info("2. Testing Pause/Resume (User Story 2)...")
    ...
    logger.info("   Pause: %ss (expected ~0s)", t_during_pause - t_before_pause:.3f)
    logger.info("   Resume: %ss (expected ~0.1s)", t_after_resume - t_during_pause:.3f)
    logger.error("   [%s] Pause/Resume", 'OK' if pause_ok and resume_ok else 'FAIL')
    logger.info(str())

    # Test 3: State update
    logger.info("3. Testing State Update...")
    ...
    logger.info("   State: ICI=%s, Coherence=%s", state['ici']:.2f, state['coherence']:.2f)
    logger.error("   [%s] State update", 'OK' if state_ok else 'FAIL')
    logger.info(str())

    # Test 4: Sync health
    logger.info("4. Testing Sync Health (SC-004)...")
    ...
    logger.info("   Max state diff: %sms", health['max_state_diff_ms']:.2f)
    logger.info("   Avg state diff: %sms", health['avg_state_diff_ms']:.2f)
    logger.error("   [%s] Sync health (SC-004: < 100ms)", 'OK' if health_ok else 'FAIL')
    logger.info(str())

    # Test 5: Latency tracking
    logger.info("5. Testing Latency Tracking (FR-003)...")
    ...
    logger.info("   Avg latency: %sms", latency_stats['avg_latency_ms']:.2f)
    logger.info("   Max latency: %sms", latency_stats['max_latency_ms']:.2f)
    logger.error("   [%s] Latency tracking (FR-003: < 50ms)", 'OK' if latency_ok else 'FAIL')
    logger.info(str())
