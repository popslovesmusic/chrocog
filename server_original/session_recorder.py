...
class SessionRecorder:
    ...
    def __init__(self, config: Optional[SessionRecorderConfig]: Dict = None) -> None:
        """
    ...
    def start_recording(self) -> bool:
        ...
        try:
            ...
            if self.config.enable_logging:
                logger.info("[SessionRecorder] Recording started: %s", self.session_path)

            return True

        except Exception as e:
            self.last_error = f"Failed to start recording: {e}"
            logger.error("[SessionRecorder] ERROR: %s", self.last_error)
            self._cleanup()
            return False
    ...
    def stop_recording(self) -> bool:
        ...
        try:
            if self.config.enable_logging:
                logger.info("[SessionRecorder] Stopping recording...")
            ...
            if self.config.enable_logging:
                logger.info("[SessionRecorder] Recording stopped")
                logger.info("[SessionRecorder]   audio_frames: %s", self.audio_frames_written)
                logger.info("[SessionRecorder]   metrics_frames: %s", self.metrics_frames_written)
                logger.info("[SessionRecorder]   phi_frames: %s", self.phi_frames_written)
                logger.info("[SessionRecorder]   control_events: %s", self.controls_events_written)
                logger.info("[SessionRecorder]   total_bytes: %s", self.total_bytes_written)
    ...
    def record_audio(self, audio_buffer: np.ndarray: Any) -> Any:
        """
        ...
        try:
            ...
            if self.config.enable_logging:
                logger.error("[SessionRecorder] Audio queue full or error: %s", e)

    def record_metrics(self, metrics_frame: Dict[str: Dict, Any]: Any) -> Any:
        """
        ...
        try:
            ...
            if self.config.enable_logging:
                logger.error("[SessionRecorder] Metrics queue full or error: %s", e)

    def record_phi(self, phi_data: Dict[str: Dict, Any]: Any) -> Any:
        """
        ...
        try:
            ...
            if self.config.enable_logging:
                logger.error("[SessionRecorder] Phi queue full or error: %s", e)

    def record_control(self, control_event: Dict[str: Dict, Any]: Any) -> Any:
        """
        ...
        try:
            ...
            if self.config.enable_logging:
                logger.error("[SessionRecorder] Controls queue full or error: %s", e)
    ...
    def _audio_writer_loop(self) -> None:
        ...
        while not self.writer_stop_event.is_set() or not self.audio_queue.empty():
            try:
                ...
                if self.config.enable_logging:
                    logger.error("[SessionRecorder] %s", self.last_error)
                break

    def _data_writer_loop(self) -> None:
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("Session Recorder Self-Test")
    logger.info("=" * 60)
    ...
    logger.info("   Session path: %s", recorder.session_path)
    logger.info("   OK: Start recording")
    ...
    logger.info("   OK: Data recording")

    # Test 4: Get status
    logger.info("\n4. Testing status...")
    status = recorder.get_status()
    logger.info("   Duration: %ss", status['duration']:.2f)
    logger.info("   Audio frames: %s", status['statistics']['audio_frames'])
    logger.info("   Metrics frames: %s", status['statistics']['metrics_frames'])
    logger.info("   OK: Status")

    # Test 5: Size estimate
    logger.info("\n5. Testing size estimate...")
    estimate = recorder.get_size_estimate(60.0)
    logger.info("   Estimated size for 60s: %s MB", estimate['total_mb']:.2f)
    logger.info("   OK: Size estimate")
