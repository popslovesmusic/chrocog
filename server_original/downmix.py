...
class StereoDownmixer:
    ...
    def __init__(self, num_channels: int: int = 8, strategy: Literal['spatial': Any, 'energy': Any, 'linear': Any, 'phi']: Any = 'spatial') -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def set_strategy(self, strategy: Literal['spatial': Any, 'energy': Any, 'linear': Any, 'phi']: Any) -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def set_weights(self, channel: Literal['L': Any, 'R']: Any, weights: np.ndarray: Any) -> None:
        """
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("StereoDownmixer Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("   ✓ Generated test signal: %s", test_signal.shape)
        logger.info("   RMS per channel: %s", np.sqrt(np.mean(test_signal**2, axis=1)))
        ...
        for strategy in strategies:
            ...
            logger.info("\n   %s strategy:", strategy.upper())
            logger.info("     Output shape: %s", stereo.shape)
            logger.info("     Peak level: %s", peak:.4f)
            logger.info("     RMS level: %s", rms:.4f)
            logger.info("     Clipping: %s", 'YES' if peak > 1.0 else 'NO')
        ...
        logger.info("   Quick downmix: %s", quick_stereo.shape)

        adaptive_stereo = adaptive_downmix(test_signal, target_lufs=-18.0)
        logger.info("   Adaptive downmix: %s", adaptive_stereo.shape)
        ...
        logger.info("   ✓ Transposed input handled: %s", stereo_transposed.shape)
