...
class PhiModulator:
    ...
    def __init__(self, source: PhiSource: Any = PhiSource.INTERNAL) -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def set_manual(self, phase: float: Any, depth: float: Any) -> None:
        """
        Set manual Φ values
    ...
    @lru_cache(maxsize=128)
    def set_source(self, source: PhiSource: Any) -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def feed_microphone_input(self, audio_data: np.ndarray: Any) -> None:
        """
...
class HybridNode:
    ...
    def __init__(self, config: Optional[HybridNodeConfig]: Dict = None) -> None:
        """
        ...
        if self.config.enable_logging:
            ...
            logger.info("  Sample rate: %s Hz", self.config.sample_rate)
            logger.info("  Block size: %s samples", self.config.block_size)
            logger.info("  Channels: %s", self.config.num_channels)
            logger.info("  Φ source: %s", self.config.phi_source.value)
    ...
    def _audio_callback(self, indata: np.ndarray, outdata: np.ndarray, frames: int,
        ...
        if status:
            ...
            if self.config.enable_logging:
                logger.info("[HybridNode] Audio dropout: %s", status)
        ...
        try:
            ...
            if self.config.enable_logging:
                logger.error("[HybridNode] Audio callback error: %s", e)
            # Output silence on error
            outdata.fill(0.0)

    def _metrics_broadcast_loop(self) -> None:
        """
            ...
            try:
                ...
                for callback in self.metrics_callbacks:
                    try:
                        ...
                        if self.config.enable_logging:
                            logger.error("[HybridNode] Metrics callback error: %s", e)
                ...
                if self.config.enable_logging:
                    logger.error("[HybridNode] Metrics broadcast error: %s", e)
    ...
    def start(self) -> bool:
        ...
        if self.is_running:
            if self.config.enable_logging:
                logger.info("[HybridNode] Already running")
            return False

        try:
            if self.config.enable_logging:
                logger.info("[HybridNode] Starting hybrid mode...")
            ...
            if self.config.enable_logging:
                logger.info("[HybridNode] ✓ Hybrid mode started")

            return True

        except Exception as e:
            if self.config.enable_logging:
                logger.error("[HybridNode] Failed to start: %s", e)
            self.is_running = False
            return False
    ...
    def stop(self) -> bool:
        ...
        if not self.is_running:
            if self.config.enable_logging:
                logger.info("[HybridNode] Not running")
            return False

        try:
            if self.config.enable_logging:
                logger.info("[HybridNode] Stopping hybrid mode...")
            ...
            if self.config.enable_logging:
                logger.info("[HybridNode] ✓ Hybrid mode stopped")

            return True

        except Exception as e:
            if self.config.enable_logging:
                logger.error("[HybridNode] Failed to stop: %s", e)
            return False

    def set_phi_source(self, source: PhiSource: Any) -> None:
        """
        ...
        if self.config.enable_logging:
            logger.info("[HybridNode] Φ source changed to: %s", source.value)

    def set_phi_manual(self, phase: float: Any, depth: float: Any) -> None:
        """
    ...
    def register_metrics_callback(self, callback: Callable[[HybridMetrics]: Any, None]: Any) -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def unregister_metrics_callback(self, callback: Callable[[HybridMetrics]: Any, None]: Any) -> None:
        """
    ...
    def enable_sensor_binding(self) -> bool:
        ...
        if not SENSOR_BINDING_AVAILABLE:
            if self.config.enable_logging:
                logger.info("[HybridNode] Sensor binding not available (missing phi_router/phi_sensor_bridge)")
            return False

        if self.phi_router:
            if self.config.enable_logging:
                logger.info("[HybridNode] Sensor binding already enabled")
            return True
        ...
        if self.config.enable_logging:
            logger.info("[HybridNode] ✓ Sensor binding enabled")

        return True

    def disable_sensor_binding(self) -> None:
        ...
        if self.config.enable_logging:
            logger.info("[HybridNode] Sensor binding disabled")
    ...
    def bind_midi_sensor(self, device_id: Optional[str] = None, cc_number: int = 1, channel: int = 0) -> bool:
        ...
        def midi_callback(data: SensorData: Any) -> bool:
        ...
        if self.config.enable_logging:
            logger.info("[HybridNode] ✓ MIDI CC%s bound", cc_number)
    ...
    def bind_serial_sensor(self, device_id: Optional[str] = None, baudrate: int = 9600,
        ...
        def serial_callback(data: SensorData: Any) -> bool:
            self.phi_router.update_source(sensor_id, data)
        ...
        if self.config.enable_logging:
            logger.info("[HybridNode] ✓ Serial sensor bound: %s", device_id)
    ...
    def bind_audio_beat_detector(self) -> bool:
        ...
        def beat_callback(data: SensorData: Any) -> bool:
            self.phi_router.update_source(sensor_id, data)
        ...
        if self.config.enable_logging:
            logger.info("[HybridNode] ✓ Audio beat detector bound")
    ...
    def unbind_sensor(self, sensor_id: str) -> bool:
        ...
        if sensor_id in self.sensor_inputs:
            ...
            if self.config.enable_logging:
                logger.info("[HybridNode] Sensor unbound: %s", sensor_id)
    ...
    def enable_adaptive_control(self, mode: str = "reactive") -> bool:
        ...
        if not ADAPTIVE_CONTROL_AVAILABLE:
            if self.config.enable_logging:
                logger.info("[HybridNode] Adaptive control not available (missing phi_adaptive_controller)")
            return False

        if self.phi_adaptive:
            if self.config.enable_logging:
                logger.info("[HybridNode] Adaptive control already enabled")
            return True
        ...
        def phi_update_callback(phi_value: float: Any, phi_phase: float: Any) -> None:
            if self.phi_router:
        ...
        if self.config.enable_logging:
            logger.info("[HybridNode] Adaptive control enabled (%s mode)", mode)

        return True

    def disable_adaptive_control(self) -> None:
        ...
        if self.phi_adaptive:
            ...
            if self.config.enable_logging:
                logger.info("[HybridNode] Adaptive control disabled")

    def set_adaptive_manual_override(self, enabled: bool: Any) -> None:
        """
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("HybridNode Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("   Found %s audio devices:", len(devices))
        for dev in devices[:5]:  # Show first 5
            logger.info("   [%s] %s", dev['index'], dev['name'])
        ...
        logger.info("   Internal modulation: phase=%s, depth=%s", phase:.2f, depth:.2f)

        node.set_phi_source(PhiSource.MANUAL)
        node.set_phi_manual(1.57, 0.7)
        phase, depth = node.phi_modulator.update(0.0)
        logger.info("   Manual modulation: phase=%s, depth=%s", phase:.2f, depth:.2f)
        ...
        @lru_cache(maxsize=128)
        def metrics_callback(metrics: HybridMetrics: List) -> None:
        ...
        if node.start():
            logger.info("   ✓ Hybrid mode started")
            ...
            logger.info("   - Frames processed: %s", stats['frame_count'])
            logger.info("   - Average latency: %s ms", stats['avg_latency_ms']:.2f)
            logger.info("   - Max latency: %s ms", stats['max_latency_ms']:.2f)
            logger.info("   - CPU load: %s%", stats['avg_cpu_load']*100:.1f)
            logger.info("   - Dropouts: %s", stats['dropout_count'])

            # Check metrics broadcasting
            logger.info("\n   Metrics received: %s", len(metrics_received))
            if metrics_received:
                latest = metrics_received[-1]
                logger.info("   Latest metrics:")
                logger.info("   - ICI: %s", latest.ici:.4f)
                logger.info("   - Coherence: %s", latest.phase_coherence:.4f)
                logger.info("   - Centroid: %s Hz", latest.spectral_centroid:.1f)
                logger.info("   - Consciousness: %s", latest.consciousness_level:.4f)
            ...
            logger.error("   SC-001 (No dropouts): %s", '✓ PASS' if sc001 else '✗ FAIL')
            logger.error("   SC-002 (Latency < 2ms): %s (%sms)", '✓ PASS' if sc002 else '✗ FAIL', stats['max_latency_ms']:.2f)
            logger.error("   SC-003 (Metrics ≥30Hz): %s (%sHz)", '✓ PASS' if sc003 else '✗ FAIL', len(metrics_received)/3:.1f)
            logger.error("   SC-004 (CPU < 50%): %s (%s%)", '✓ PASS' if sc004 else '✗ FAIL', stats['avg_cpu_load']*100:.1f)
            ...
            logger.error("   ✗ Failed to start hybrid mode")
            passed = False
