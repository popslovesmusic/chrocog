...
try:
    ...
    logger.warning("[PhiSensorBridge] Warning: mido not available. MIDI support disabled.")
...
try:
    ...
    logger.warning("[PhiSensorBridge] Warning: pyserial not available. Serial sensor support disabled.")
...
class MIDIInput:
    ...
    def __init__(self, config: SensorConfig: Dict, callback: Callable[[SensorData]: Any, None]: Any) -> None:
        """
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if not available_ports:
                logger.info("[MIDIInput] No MIDI input devices found")
                return False
            ...
            if self.config.device_id:
                if self.config.device_id in available_ports:
                    ...
                    logger.info("[MIDIInput] Device '%s' not found", self.config.device_id)
                    return False
            else:
    ...
    def _midi_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                if self.config.enable_logging:
                    logger.error("[MIDIInput] Error in MIDI loop: %s", e)
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.config.device_id:
                ...
                if not ports:
                    logger.info("[SerialSensor] No serial ports found")
                    return False
                port = ports[0].device
            ...
            if self.config.enable_logging:
                logger.info("[SerialSensor] Connected to: %s @ %s baud", port, self.config.serial_baudrate)

            return True

        except Exception as e:
            logger.error("[SerialSensor] Failed to connect: %s", e)
            return False
    ...
    def _serial_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                if self.config.enable_logging:
                    logger.error("[SerialSensor] Error in serial loop: %s", e)
...
class AudioBeatDetector:
    ...
    @lru_cache(maxsize=128)
    def process_audio(self, audio_block: np.ndarray: Any, sample_rate: int: Any = 48000) -> None:
        """
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("PhiSensorBridge Self-Test")
    logger.info("=" * 60)
    ...
    if MIDI_AVAILABLE:
        ...
        logger.info("   Found %s MIDI devices:", len(devices))
        for dev in devices:
            logger.info("   - %s", dev)
    else:
        logger.info("   MIDI not available (install mido)")

    # Test Serial device listing
    logger.info("\n2. Testing Serial device listing...")
    if SERIAL_AVAILABLE:
        devices = SerialSensorInput.list_devices()
        logger.info("   Found %s serial devices:", len(devices))
        for dev in devices:
            logger.info("   - %s", dev)
    else:
        logger.info("   Serial not available (install pyserial)")
    ...
    def beat_callback(data: SensorData: Any) -> bool:
        ...
        logger.info("   Beat detected! Strength: %s, Φ: %s", data.raw_value:.2f, data.normalized_value:.3f)
