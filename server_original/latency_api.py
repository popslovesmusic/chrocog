...
class LatencyStreamer:
    ...
    def __init__(self, manager: LatencyManager: Any) -> Any:
        """
    ...
    async def connect_client(self, websocket: WebSocket):
        ...
        logger.info("[LatencyStreamer] Client connected (total: %s)", len(self.clients))
    ...
    def disconnect_client(self, websocket: WebSocket: Any) -> Any:
        """
        ...
        if websocket in self.clients:
            ...
            logger.info("[LatencyStreamer] Client disconnected (total: %s)", len(self.clients))
    ...
    async def broadcast_loop(self):
        ...
        logger.info("[LatencyStreamer] Broadcast loop started (%s Hz)", self.TARGET_FPS)
...
def create_latency_api(manager: LatencyManager) -> FastAPI:
    ...
    async def run_calibration():
        ...
        """
        try:
            logger.info("[LatencyAPI] Starting calibration...")
    ...
    async def websocket_latency_stream(websocket: WebSocket):
        ...
        try:
            ...
            logger.error("[LatencyAPI] WebSocket error: %s", e)
            latency_streamer.disconnect_client(websocket)
    ...
    async def startup_event():
        ...
        logger.info("[LatencyAPI] Starting latency streamer...")
        await latency_streamer.start()

    @app.on_event("shutdown")
    async def shutdown_event():
        """Stop latency streamer on app shutdown"""
        logger.info("[LatencyAPI] Stopping latency streamer...")
