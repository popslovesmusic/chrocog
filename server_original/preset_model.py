...
class Preset:
    ...
    def diff(self, other: 'Preset') -> Dict:
        ...
        def compare_recursive(path: str: str, val1: Any, val2: Any) -> Any:
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("Preset Model Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("   Created: %s (ID: %s...)", preset.name, preset.id[:8])
        ...
        logger.info("   Cloned with new ID: %s...", preset_copy.id[:8])
        ...
        logger.info("   Changes detected: %s", len(changes))
        ...
        default.validate()
        logger.info("   Default preset: %s", default.name)
