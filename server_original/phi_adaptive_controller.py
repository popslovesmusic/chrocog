...
class PhiAdaptiveController:
    ...
    def __init__(self, config: Optional[AdaptiveConfig]: Dict = None) -> None:
        """
    ...
    def set_phi_update_callback(self, callback: Callable[[float: Any, float]: Any, None]: Any) -> None:
        """
    ...
    def enable(self, mode: AdaptiveMode: Any = AdaptiveMode.REACTIVE) -> Any:
        """
        ...
        with self.lock:
            ...
            if self.config.enable_logging:
                logger.info("[PhiAdaptive] Enabled in %s mode", mode.name)
    ...
    def disable(self) -> Any:
        ...
        with self.lock:
            ...
            if self.config.enable_logging:
                logger.info("[PhiAdaptive] Disabled")

    def set_manual_override(self, enabled: bool: Any) -> None:
        """
        ...
        with self.lock:
            ...
            if self.config.enable_logging:
                logger.info("[PhiAdaptive] Manual override: %s", 'ON' if enabled else 'OFF')
    ...
    def _control_loop(self) -> None:
        ...
        while self.is_running:
            ...
            try:
                ...
                if self.config.enable_logging:
                    logger.error("[PhiAdaptive] Control loop error: %s", e)
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("PhiAdaptiveController Self-Test")
    logger.info("=" * 60)
    logger.info(str())

    # Create controller
    logger.info("1. Creating PhiAdaptiveController...")
    ...
    logger.info("   [OK] Controller created")
    logger.info(str())

    # Track Phi updates
    phi_updates = []

    def phi_callback(phi: Any, phase: Any) -> None:
        phi_updates.append((time.time(), phi, phase))

    controller.set_phi_update_callback(phi_callback)

    # Enable adaptive control
    logger.info("2. Enabling adaptive control (REACTIVE mode)...")
    controller.enable(AdaptiveMode.REACTIVE)
    time.sleep(0.2)
    status = controller.get_status()
    logger.info("   [OK] Enabled: %s, Mode: %s", status.is_enabled, status.mode.name)
    logger.info(str())

    # Simulate metric updates
    logger.info("3. Simulating metric updates for 2 seconds...")
    ...
    logger.info("   [OK] Sent %s metric updates", update_count)
    logger.info("   [OK] Received %s Phi updates", len(phi_updates))
    logger.info(str())

    # Check status
    logger.info("4. Checking adaptive control status...")
    status = controller.get_status()
    logger.info("   ICI: %s (target: %s)", status.current_ici:.3f, status.target_ici:.3f)
    logger.error("   ICI error: %s", status.ici_error:.3f)
    logger.info("   Current Phi: %s", status.current_phi:.3f)
    logger.info("   Predicted Phi: %s", status.predicted_phi:.3f)
    logger.info("   Update count: %s", status.update_count)
    logger.info("   Avg latency: %s ms", status.avg_update_latency_ms:.2f)
    ...
    logger.info("   Override latency: %s ms", override_latency:.2f)
    ...
    controller.disable()
    status = controller.get_status()
    logger.info("   [OK] Disabled: %s", not status.is_enabled)
    logger.info(str())
    ...
    if stats:
        ...
        logger.info("   Sample count: %s", stats['sample_count'])
