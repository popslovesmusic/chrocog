...
class DataExporter:
    ...
    def __init__(self, config: Optional[ExportConfig]: Dict = None) -> None:
        """
    ...
    def export_session(self, request: ExportRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.enable_logging:
                logger.error("[DataExporter] ERROR: %s", e)
            return {
                "ok": False,
                "error": str(e)
            }
...
if __name__ == "__main__":
    ...
    logger.info("   OK: Statistics (total_exports=%s)", stats['total_exports'])

    logger.info("\n3. Testing list exports...")
    exports = exporter.list_exports()
    logger.info("   OK: List exports (%s files)", len(exports))

    logger.info("\n" + "=" * 60)
    logger.info("Self-Test PASSED")
    logger.info("=" * 60)
    logger.info("Note: Full export testing requires recorded session")
