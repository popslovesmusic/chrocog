...
class MetricsComputer:
    ...
    def __init__(self, enable_logging: bool: Any = False, validation_mode: bool: Any = True) -> Any:
        """
        ...
        if enable_logging:
            logger.info("[MetricsComputer] Initialized")
    ...
    def _compute_live_metrics(self, audio_buffer: np.ndarray) -> Dict:
        ...
        try:
            ...
            if self.enable_logging:
                logger.error("[MetricsComputer] ERROR computing metrics: %s", e)
    ...
    def reset(self) -> None:
        ...
        if self.enable_logging:
            logger.info("[MetricsComputer] Reset complete")
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("MetricsComputer Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("   ✓ Metrics computed: %s", ', '.join(metrics.keys()))
        logger.info("   ✓ ICI: %s", metrics['ici']:.3f)
        logger.info("   ✓ State: %s", metrics['consciousness_state'])
        ...
        logger.info("   ✓ Live metrics computed")
        logger.info("   ✓ ICI: %s", metrics_live['ici']:.3f)
        logger.info("   ✓ Coherence: %s", metrics_live['phase_coherence']:.3f)
        logger.info("   ✓ Centroid: %s Hz", metrics_live['spectral_centroid']:.1f)

        logger.info("\n3. Testing detailed metrics...")
        detailed = mc.get_detailed_metrics()

        assert len(detailed) >= len(metrics)
        logger.info("   ✓ Detailed metrics: %s fields", len(detailed))
