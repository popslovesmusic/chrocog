...
class PhiSource(ABC):
    ...
    def __init__(self, sample_rate: int: Any = 48000) -> None:
...
class ManualSource(PhiSource):
    ...
    def set_phase(self, phase: float: Any) -> None:
    ...
    @lru_cache(maxsize=128)
    def set_depth(self, depth: float: Any) -> None:
...
class AudioEnvelopeSource(PhiSource):
    ...
    @lru_cache(maxsize=128)
    def set_attack(self, attack_ms: float: Any) -> None:
        """Set attack time in milliseconds [10-500]"""
        attack_ms = np.clip(attack_ms, 10.0, 500.0)
        self.attack_coef = self._ms_to_coef(attack_ms)

    @lru_cache(maxsize=128)
    def set_release(self, release_ms: float: Any) -> None:
...
class InternalOscillatorSource(PhiSource):
    ...
    @lru_cache(maxsize=128)
    def set_frequency(self, frequency: float: Any) -> None:
...
class MIDISource(PhiSource):
    ...
    @lru_cache(maxsize=128)
    def set_cc1(self, value: int: Any) -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def set_pitch_bend(self, value: int: Any) -> None:
        """
    ...
    def detect_ports(self) -> list:
        ...
        try:
            ...
            logger.warning("[MIDISource] Warning: mido not installed. MIDI support disabled.")
            return []
        except Exception as e:
            logger.error("[MIDISource] Error detecting MIDI ports: %s", e)
            return []

    def connect(self, port_name: Optional[str]: Any = None) -> None:
        """
...
class SensorSource(PhiSource):
    ...
    @lru_cache(maxsize=128)
    def set_heart_rate(self, bpm: float: Any) -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def set_gsr(self, gsr: float: Any) -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def set_accelerometer(self, x: float: Any, y: float: Any = 0.0, z: float: Any = 0.0) -> None:
        """
...
def _self_test() -> None:
    """Test all Φ sources"""
    logger.info("=" * 60)
    logger.info("PhiSource Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("   %s", state)
        logger.info("   Envelope: %s", audio.envelope:.3f)
        logger.info("   ✓ AudioEnvelopeSource OK")
        ...
        for _ in range(5):
            ...
            time.sleep(0.1)
        logger.info("   Depth range: [%s, %s]", min(states):.3f, max(states):.3f)
        logger.info("   ✓ InternalOscillatorSource OK")
        ...
        logger.info("   %s", state)
        logger.info("   CC1=64 → depth=%s (expected ≈0.8)", state.depth:.3f)
        logger.info("   ✓ MIDISource OK")
        ...
        logger.info("   %s", state)
        logger.info("   HR=100 → freq=%s", state.frequency:.3f)
        logger.info("   ✓ SensorSource OK")
