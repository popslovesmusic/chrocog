...
class PhiModulator:
    ...
    def __init__(self, sample_rate: int: Any = 48000) -> None:
        ...
        logger.info("[PhiModulator] Φ-frequency: %s Hz", self.phi_frequency:.2f)
    ...
    def reset(self) -> None:
        ...
        self.phase_accumulator = 0.0
        logger.info("[PhiModulator] Phase accumulator reset")
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("PhiModulator Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("   ✓ Modulator initialized")
        ...
        logger.info("   Range: [%s, %s]", np.min(mod_single):.3f, np.max(mod_single):.3f)
        logger.info("   Mean: %s", np.mean(mod_single):.3f)
        ...
        logger.info("   Amplitude modulation: %s", am_output.shape)

        ring_output = modulator.applyModulation(test_signal, test_mod, mode='ring')
        logger.info("   Ring modulation: %s", ring_output.shape)
        ...
        logger.info("   Phases (radians): %s", fib_phases)
        logger.info("   Phases (degrees): %s", np.degrees(fib_phases))
