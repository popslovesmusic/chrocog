...
class SyncProfiler:
    ...
    def __init__(self, config: Optional[ProfilerConfig]: Dict = None) -> None:
    ...
    def handle_pong_message(self, pong_msg: Dict: Dict) -> Any:
        """
        ...
        if ping_id not in self.pending_pings:
            if self.config.enable_logging:
                logger.info("[SyncProfiler] Received pong for unknown ping_id: %s", ping_id)
            return
    ...
    def cleanup_pending_pings(self, timeout_s: float: Any = 1.0) -> None:
        """
        ...
        for ping_id in timed_out:
            ...
            if self.config.enable_logging:
                logger.info("[SyncProfiler] Ping %s timed out", ping_id)
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("SyncProfiler Self-Test")
    logger.info("=" * 60)
    logger.info(str())
    ...
    logger.info("   Initial state: pings=%s, pongs=%s", profiler.total_pings_sent, profiler.total_pongs_received)
    logger.error("   [%s] Initialization", 'OK' if init_ok else 'FAIL')
    logger.info(str())

    # Test 2: Create ping message
    logger.info("2. Testing Ping Message Creation...")
    ...
    logger.info("   Ping message: %s", ping_msg)
    logger.info("   Total pings sent: %s", profiler.total_pings_sent)
    logger.error("   [%s] Ping creation", 'OK' if ping_ok else 'FAIL')
    logger.info(str())

    # Test 3: Handle pong message
    logger.info("3. Testing Pong Message Handling...")
    ...
    if profiler.measurements:
        ...
        logger.info("   RTT: %sms", m.round_trip_ms:.2f)
        logger.info("   One-way: %sms", m.one_way_ms:.2f)
        logger.info("   Jitter: %sms", m.jitter_ms:.2f)

    logger.error("   [%s] Pong handling", 'OK' if pong_ok else 'FAIL')
    logger.info(str())

    # Test 4: Multiple measurements
    logger.info("4. Testing Multiple Measurements...")
    ...
    logger.info("   Total measurements: %s", len(profiler.measurements))
    logger.info("   Success rate: %s%", profiler.total_pongs_received / profiler.total_pings_sent * 100:.1f)
    logger.error("   [%s] Multiple measurements", 'OK' if multi_ok else 'FAIL')
    logger.info(str())

    # Test 5: Statistics
    logger.info("5. Testing Statistics...")
    ...
    logger.info("   Avg latency: %sms", stats['avg_latency_ms']:.2f)
    logger.info("   Min latency: %sms", stats['min_latency_ms']:.2f)
    logger.info("   Max latency: %sms", stats['max_latency_ms']:.2f)
    logger.info("   P95 latency: %sms", stats['p95_latency_ms']:.2f)
    logger.info("   P99 latency: %sms", stats['p99_latency_ms']:.2f)
    logger.info("   Avg jitter: %sms", stats['avg_jitter_ms']:.2f)
    logger.info("   Success rate: %s%", stats['success_rate']:.1f)
    logger.info("   Meets SC-001: %s", stats['meets_sc001'])
    logger.info("   Meets SC-003: %s", stats['meets_sc003'])
    logger.error("   [%s] Statistics", 'OK' if stats_ok else 'FAIL')
    logger.info(str())

    # Test 6: Latency distribution
    logger.info("6. Testing Latency Distribution...")
    ...
    logger.info("   Distribution buckets: %s", len(dist['buckets']))
    for i in range(min(5, len(dist['buckets']))):
        logger.info("     %s: %s (%s%)", dist['buckets'][i], dist['counts'][i], dist['percentages'][i]:.1f)

    logger.error("   [%s] Distribution", 'OK' if dist_ok else 'FAIL')
    logger.info(str())

    # Test 7: Timeout handling
    logger.info("7. Testing Timeout Handling...")
    ...
    logger.info("   Total timeouts: %s", profiler.total_timeouts)
    ...
    if all_ok:
        ...
        logger.error("Self-Test FAILED - Review failures above")
    logger.info("=" * 60)
