...
class MetricsStreamer:
    ...
    def submit_frame(self, frame: MetricsFrame: Any) -> None:
        """
    ...
    async def stop_broadcasting(self):
        ...
        logger.info("[MetricsStreamer] Broadcasting stopped")
    ...
    async def close(self):
        ...
        logger.info("[MetricsStreamer] Shutting down")
...
def _self_test() -> None:
    ...
    logger.info("=" * 60)
    logger.info("MetricsStreamer Self-Test")
    logger.info("=" * 60)
    ...
    try:
        ...
        async def run_test():
            ...
            logger.info("   ✓ Submitted %s frames", streamer.frame_counter)

            # Check latest frame
            logger.info("\n4. Checking latest frame...")
            latest = streamer.get_latest_frame()
            logger.info("   Latest frame: %s", latest)
            assert latest is not None
            logger.info("   ✓ Latest frame OK")

            # Get statistics
            logger.info("\n5. Getting statistics...")
            stats = streamer.get_statistics()
            logger.info("   Frames sent: %s", stats['total_frames_sent'])
