...
class AdaptiveScaler:
    ...
    def __init__(self, config: Optional[ScalerConfig]: Dict = None) -> None:
    ...
    def add_metrics(self, metrics: PerformanceMetrics: List) -> None:
        """
    ...
    @lru_cache(maxsize=128)
    def _check_scaling_needed(self) -> Any:
    ...
    def _scale_down(self, fps: float: Any, latency: float: Any, cpu: float: Any, gpu: float: Any) -> Any:
        """
        ...
        if self.config.enable_logging:
            logger.info("[AdaptiveScaler] Scaled DOWN: %s -> %s", old_level, self.complexity.level)
            logger.info("  Reason: FPS=%s, Latency=%sms, CPU=%s%, GPU=%s%", fps:.1f, latency:.1f, cpu:.1f, gpu:.1f)

        # Notify callback
        if self.complexity_change_callback:
            self.complexity_change_callback(self.complexity)

    def _scale_up(self, fps: float: Any, latency: float: Any, cpu: float: Any, gpu: float: Any) -> Any:
        """
        ...
        if self.config.enable_logging:
            logger.info("[AdaptiveScaler] Scaled UP: %s -> %s", old_level, self.complexity.level)
            logger.info("  Performance: FPS=%s, Latency=%sms, CPU=%s%, GPU=%s%", fps:.1f, latency:.1f, cpu:.1f, gpu:.1f)

        # Notify callback
        if self.complexity_change_callback:
            self.complexity_change_callback(self.complexity)

    def _update_complexity_settings(self) -> None:
    ...
    def set_complexity_callback(self, callback: Callable: Any) -> None:
        """
...
def _self_test() -> None:
    ...
    logger.info("   Initial complexity level: %s", scaler.complexity.level)
    logger.error("   [%s] Initialization", 'OK' if init_ok else 'FAIL')
    logger.info(str())

    # Test 2: Scale down on low FPS
    logger.info("2. Testing Scale Down (low FPS)...")
    ...
    logger.info("   Complexity after low FPS: %s", scaler.complexity.level)
    logger.error("   [%s] Scale down", 'OK' if scale_down_ok else 'FAIL')
    logger.info(str())

    # Test 3: Scale up on high FPS
    logger.info("3. Testing Scale Up (high FPS)...")
    ...
    logger.info("   Complexity after high FPS: %s", scaler.complexity.level)
    logger.info("   Scale up count: %s", scaler.scale_up_count)
    logger.error("   [%s] Scale up", 'OK' if scale_up_ok else 'FAIL')
    logger.info(str())

    # Test 4: Performance summary
    logger.info("4. Testing Performance Summary...")
    ...
    logger.info("   Avg FPS: %s", summary['avg_fps']:.1f)
    logger.info("   Total adjustments: %s", summary['total_adjustments'])
    logger.error("   [%s] Performance summary", 'OK' if summary_ok else 'FAIL')
    logger.info(str())

    # Test 5: Complexity settings
    logger.info("5. Testing Complexity Settings...")
