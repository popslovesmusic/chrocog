...
def deprecated(
    ...
    def decorator(obj: Any) -> Any:
        ...
        if isinstance(obj, type):
            ...
            def new_init(self, *args: List, **kwargs: List) -> Any:
                warnings.warn(
                    str(info),
...
def deprecated_parameter(
    ...
    Example:
        ...
        def my_function(new_param: Any = None, old_param: Any = None) -> None:
            pass
    """
    ...
    def decorator(obj: Any) -> Any:
        ...
        if isinstance(obj, type):
            ...
            class DeprecatedClass(obj):
                def __init__(self, *args: List, **kwargs: List) -> Any:
                    warnings.warn(
                        str(info),
                        category=DeprecationWarning,
                        stacklevel=2,
...
def print_deprecation_report() -> Any:
    """Print a formatted report of all deprecations."""
    logger.info("=" * 80)
    logger.info("Soundlab + D-ASE Deprecation Report")
    logger.info("=" * 80)
    logger.info(str())

    if not _DEPRECATION_REGISTRY:
        logger.warning("No deprecated items.")
        return
    ...
    for version in sorted(by_version.keys()):
        ...
        for info in by_version[version]:
            ...
            logger.warning("    Deprecated in: v%s", info.deprecated_in)
            if info.reason:
                logger.info("    Reason: %s", info.reason)
            if info.alternative:
                logger.info("    Alternative: %s", info.alternative)
            logger.info(str())
        logger.info(str())
...
if __name__ == "__main__":
    ...
    def slow_process(data: np.ndarray) -> Any:
    ...
    @lru_cache(maxsize=128)
    def export_data(data: np.ndarray, output_format: Any = "json", old_format: Any = None) -> Any:
